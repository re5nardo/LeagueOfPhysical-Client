...
public class Grid : GameFramework.Grid
{
    public override void Add(int entityId, bool bPublish = true)
    {
        ...
        if (cells[vec2CellPosition].Add(entityId))
        {
            ...
            if (bPublish)
            {
                //GridPubSubService.Instance.Publish(MessageKey.EntityAddedToGrid, entityId, vec2CellPosition);
            }
        }
    }
    ...
    public override void Remove(int entityId, bool bPublish = true)
    {
        if (cells[entityIdCellPosition[entityId]].Remove(entityId))
        {
            ...
            if (bPublish)
            {
                //GridPubSubService.Instance.Publish(MessageKey.EntityRemovedFromGrid, entityId, vec2CellPosition);
            }
        }
    }
    ...
    public override void Move(int entityId)
    {
        ...
        if (pre != cur)
        {
            ...
            //GridPubSubService.Instance.Publish(MessageKey.EntityMoveCell, entityId, pre, cur);
        }
    }
